...
#         loss = np.mean((y_pred - y)**2) + reg *(np.sum(W1*W1) + np.sum(W2*W2))
#         grad_y_pred = -2*(y_pred-y)
#         local_grad = np.zeros_like(probs)
#         local_grad[np.arange(len(y_pred)), y_pred] = 1
#         argmax_grad = local_grad*grad_y_pred.reshape((len(y_pred), 1))/len(y)
